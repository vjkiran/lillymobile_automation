...
d['assertIsNone']           = ('assert',            ' is None', [1,2])
d['assertIsNotNone']        = ('assert',            ' is not None', [1,2])
d['assertGreater']          = ('assert',            ' >', [2,3])

#  the list of synonyms
d['failUnlessRaises']      = d['assertRaises']
d['failUnless']            = d['assert_']
d['assertTrue']            = d['assert_']
d['assertFalse']           = d['failIf']
...
def rewrite_utest(block):
    ...
    elif old in ['assertIsNone', 'assertIsNotNone']:
        string = ' ' + ''.join(argl) + op
